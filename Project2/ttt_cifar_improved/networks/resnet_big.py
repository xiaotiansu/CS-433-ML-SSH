...
class JointResNet(nn.Module):
    ...
    def __init__(self, name='resnet50', feat_dim=128, num_classes=10):
        ...
        if num_classes > 999:
            # fixed everything but the ssl head
            set_parameter_requires_grad(self.encoder, True)
            set_parameter_requires_grad(self.fc, True)
            print('JointResNet: Freeze everything but the ssl head')
        else:
            print('JointResNet: Train everything')
